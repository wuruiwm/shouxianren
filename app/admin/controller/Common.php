...
use app\admin\service\ImageTool;
...
//                'data'=>[
//                    'id' => $is_exist['id'],
//                    'src' => $is_exist['filepath']
//                ]
//            ]);
//        }
        $info = $files->move(ROOT_PATH . 'public' . DS . 'uploads' . DS . 'admin');
        $filepath = DS . 'uploads' . DS . 'admin' . DS . $info->getSaveName();//文件路径
        $post = input('post.');
        mkdirs('./uploads/admin/original/'.timeToStr(time(),'Ymd'));
        $thumb_path = './uploads/admin/original/'.timeToStr(time(),'Ymd').'/'.$info->getFilename();//文件路径

         Image::open($_SERVER["DOCUMENT_ROOT"].$filepath)->thumb($post['width'],$post['height'],$post['type'])->save($thumb_path,$type=null,$quality=100,$interlace=false);
        if ($info) {
            //写入到附件表
            $data = [];
            $data['module'] = 'admin';
            $data['filename'] = $info->getFilename();//文件名
            $data['filepath'] = DS . 'uploads' . DS . 'admin' . DS .'original/'. $info->getSaveName();//缩略图
            $data['original_image'] = DS . 'uploads' . DS . 'admin' . DS . $info->getSaveName();//原图
            $data['fileext'] = $info->getExtension();//文件后缀
            $data['filesize'] = $info->getSize();//文件大小
            $data['create_time'] = time();//时间
            $data['uploadip'] = $this->request->ip();//IP
            ...
            $res['id'] = Db::name('attachment')->insertGetId($data);
            $res['src'] = DS . 'uploads' . DS . 'admin' . DS .'original/'. $info->getSaveName();
            throw new SuccessMessage([
                'data'=>$res
            ]);
        } else {
